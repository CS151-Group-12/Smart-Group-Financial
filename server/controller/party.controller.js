...
partyController.get('/:id', (req, res) => {
  ...
  db.query(query, (err, data) => {
    if (err) {
      console.log('Error while performing Party Query.' + err);
    } else {
      const returnData = [...data];
      res.status(200).json(returnData);
    }
...
partyController.post('/', (req, res) => {
  ...
  db.query(createPartyQuery, (createPartyErr, createParty) => {
    ...
    const userJoinParty = `INSERT INTO User_Join_Party values('${userID}', ${createParty.insertId})`;
    db.query(userJoinParty, (userJoinPartyErr, data) => {
      if (userJoinPartyErr) res.status(500).json(userJoinPartyErr);
    });
    ...
    db.query(userFormParty, (userFormPartyErr, data) => {
      ...
      const getPary = `SELECT * FROM Party WHERE partyID = '${createParty.insertId}'`;
      db.query(getPary, (getPartyErr, party, userFields) => {
        if (getPartyErr) res.status(500).json(getPartyErr);
        else {
...
partyController.post('/join', (req, res) => {
  ...
  const getPartyByNameQuery = `SELECT partyID FROM Party WHERE name like '${name}'`;
  ...
  db.query(getPartyByNameQuery, (err, foundParty) => {
    ...
    const joinPartyQuery = `INSERT INTO User_Join_Party values('${userID}', '${foundParty[0].partyID}')`;

    db.query(joinPartyQuery, (err1, data) => {
      if (err1) return res.status(500).json(err1);
      const getPary = `SELECT * FROM Party WHERE partyID = '${foundParty[0].partyID}'`;
